...
def run_inference(task_model, AENet, adaptors, dataset, opt, save_dir, thr=0):
    ...
    for idx, data in tqdm(enumerate(dataset)):
        ...
        if rec_loss_value > threshold_before:
            used_comb, ssim_score, mae_score, psnr_score, min_loss, selected_loss = TTA_rndm_50(
                adaptors, opt, task_model, save_dir, data, rec_loss, return_layers=opt.return_layers, psnr_score_no_tta=psnr_score
            )
...
if __name__ == '__main__':
    ...
    output_dir = os.path.join(opt.results_dir.replace('_rec_models', '_TTA'), 'TTA_ema', dataset_name)
