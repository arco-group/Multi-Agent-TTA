...
def calcola_unpaired(metrica, immagine):
   new=immagine.unsqueeze(0)
   if metrica=='niqe':
      niqe = pyiqa.create_metric('niqe', device=torch.device("cuda:0"), as_loss=True)                 #non la uso come funzione di perdita
      niqe_score = niqe(new).item()
      return niqe_score
   if metrica=='brisque':
      brisque=pyiqa.create_metric('brisque', device=torch.device("cuda:0"), as_loss=True) 
      brisque_score=brisque(new).item()
      return brisque_score
   if metrica=='paq2piq':
      new_3d=change_3d(new)
      paq2piq = pyiqa.create_metric('paq2piq', device=torch.device("cuda:0"), as_loss=True) 
      paq2piq_score=paq2piq(new_3d).item()
      return paq2piq_score
   if metrica=='ilniqe':
      new_3d=change_3d(new)
      il_niqe=pyiqa.create_metric('ilniqe', device=torch.device("cuda:0"), as_loss=True) 
      il_score=il_niqe(new_3d).item()
      return il_score 
   if metrica=='musiq-ava':
      new_3d=change_3d(new)
      musiq_ava=pyiqa.create_metric('musiq-ava', device=torch.device("cuda:0"), as_loss=True) 
      musiq_ava_score=musiq_ava(new_3d).item()
      return musiq_ava_score
   if metrica=='musiq-koniq':
      new_3d=change_3d(new)
      musiq_koniq=pyiqa.create_metric('musiq-koniq', device=torch.device("cuda:0"), as_loss=True) 
      musiq_koniq_score=musiq_koniq(new_3d).item()
      return musiq_koniq_score
   if metrica=='musiq-paq2piq':
      new_3d=change_3d(new)
      musiq_paq2piq=pyiqa.create_metric('musiq-paq2piq', device=torch.device("cuda:0"), as_loss=True) 
      musiq_paq2piq_score=musiq_paq2piq(new_3d).item()
      return musiq_paq2piq_score
   if metrica=='musiq-spaq':
      new_3d=change_3d(new)
      musiq_spaq=pyiqa.create_metric('musiq-spaq', device=torch.device("cuda:0"), as_loss=True) 
      musiq_spaq_score=musiq_spaq(new_3d).item()
      return musiq_spaq_score
   if metrica=='dbcnn':
      new_3d=change_3d(new)
      dbcnn=pyiqa.create_metric('dbcnn', device=torch.device("cuda:0"), as_loss=True) 
      dbcnn_score=dbcnn(new_3d).item()
      return dbcnn_score
   if metrica=='hyperiqa':
      new_3d=change_3d(new)
      hyper=pyiqa.create_metric('hyperiqa', device=torch.device("cuda:0"), as_loss=True) 
      hyper_score=hyper(new_3d).item()
      return hyper_score
   if metrica=='clipiqa':
      new_3d=change_3d(new)
      clip=pyiqa.create_metric('clipiqa', device=torch.device("cuda:0"), as_loss=True) 
      clip_score=clip(new_3d).item()
      return clip_score
   if metrica=='nima':
      new_3d=change_3d(new)
      nima=pyiqa.create_metric('nima', device=torch.device("cuda:0"), as_loss=True) 
      nima_score=nima(new_3d).item()
      return nima_score
