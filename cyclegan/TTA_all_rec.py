...
def TTA_rndm_50(adaptors, opt, task_model, save_dir, batch, rec_loss, stable=False, return_layers=None, plot=False, psnr_score_no_tta=0.0):
    ...
    for comb in tutte_combinazioni:
        ...
        for epoch in range(opt.sepochs):
            ...
            loss = adaptors.ALoss(outputs['input'],
                                    task_model.real_A)  # calcola la loss tra l'immagine di input e adattata
        ...
        with torch.no_grad():
            outputs = adaptors(batch, task_model, opt.model)  # passo forward degli adaptor
...
def TTA_all(adaptors, opt, task_model, save_dir, batch, rec_loss, stable=False, return_layers=None, plot=False, psnr_score_no_tta=0.0):
    n = len(return_layers[1:-1])  # i ricostruttori escluso il primo e l'ultimo
    chosen_comb = list(range(n))  # usa tutti i layer intermedi
    ...
    # imposta return_layers con tutti i layer
    comb_to_print = chosen_comb
    chosen_comb = [0] + [x + 1 for x in chosen_comb] + [n + 1]
    chosen_comb = [return_layers[i] for i in chosen_comb]
    opt.return_layers = chosen_comb
    ...
    for epoch in range(opt.tepochs):  # numero di iterazioni per ogni campione
        ...
        loss = 0  # somma delle loss (una per ogni adaptor)
        print('---------------------------------')
    ...
    selected_loss_output = float(loss_output[min_index])
    min_loss = float(loss_tot[min_index] / len(comb_to_print))
